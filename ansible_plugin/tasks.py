...
def download_playbook(playbook_file, target_file, **kwargs):
    ...
    try:
        blueprint, resource_path, log, target_path = ctx.download_resource(
            playbook_file, target_file)
    except Exception as e:
        raise exceptions.NonRecoverableError(
            "Could not get '{0}' ({1}: {2})".format(
                playbook_file, type(e).__name__, e))

    ctx.logger.info(
        "Blueprint: {0}, Log: {1}, Downloaded playbook "
        " file '{2}' to: {3}. ".format(
            blueprint, resource_path, log, target_path))
